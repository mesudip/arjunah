...
- `src/renderer/core.js`: the chat surface itself (panel, transcript, activity feed, transcript cards, progress lines, thread panel, composer, drag) as a dependency-free classic script loaded beside `content.js`. It knows nothing about grants, providers, or the extension APIs, and `scripts/check.mjs` enforces that. `content.js` keeps everything wallet-specific and fills the renderer's three slots. `packages/widget/` publishes the same file with an ESM footer plus a backend client, built by `scripts/build-widget.mjs`; the static check fails if the published renderer drifts from the extension's.
...
- Version bump touches, all enforced by `scripts/check.mjs`: `src/lib/constants.js` VERSION, `src/page-api.js` version, both manifests' `version` plus the Chrome `version_name` (carries the prerelease tag), `desktop/lib/server.mjs` APP_VERSION and PROTOCOL_VERSION, the four `package.json` versions (identical, may carry `-alpha.N`), `packages/sdk/src/index.ts` PROTOCOL_VERSION and `src/types.ts` version literal, the `version` assertions in `tests/e2e/chrome.mjs`, `tests/e2e/firefox.mjs`, `tests/background.test.mjs`, and `desktop/lib/providers/codex.mjs`, and SPEC.md's header and section 3. Three version strings, three shapes: npm packages carry the full prerelease (`1.2.0-alpha.1`), every runtime string (VERSION, both manifests' `version`, page API, SDK, desktop, SPEC section 3's IDL comment and `arjunah:ready` detail) is the bare protocol version (`1.2.0`), and SPEC.md's header carries the channel without its number (`Version: **1.2.0-alpha**`). Do not drop the channel from SPEC.md when bumping; `scripts/check.mjs` now fails if you do. A release tag must equal `v` + package version or the release workflow fails.
